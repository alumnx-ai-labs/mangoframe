...
model = YOLO("our_ownversion001.pt")   # your trained mango model

# Set confidence threshold for mango tree detection
...
with open(CSV_PATH, "w", newline="") as csvfile:
    ...
    writer.writerow(["video_file", "frame", "track_id", "x1", "y1", "x2", "y2", "confidence", "lat", "lon", "image_path"])
    ...
    for video_file in video_files:
        ...
        while True:
            ...
            # Run YOLOv8 detection + tracking with confidence threshold
            results = model.track(frame, persist=True, tracker=TRACKER_CONFIG, conf=CONFIDENCE_THRESHOLD)
            ...
            if results and len(results[0].boxes) > 0:
                ...
                # Filter for mango trees only (class 0 in single-class model)
                classes = results[0].boxes.cls.cpu().numpy().astype(int)
                
                for box, confidence, track_id, cls in zip(boxes, confidences, ids, classes):
                    # Only process mango tree detections (class 0)
                    if cls == 0:  # mango_tree class
                        x1, y1, x2, y2 = map(int, box)
                        lat, lon = gps_data.get(frame_num, (None, None))

                        # ============================
                        # Draw box + ID + GPS + Confidence (Green for mango trees)
                        # ============================
                        cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 4)  # Green for mango trees

                        # Track ID and confidence label
                        label = f"Mango Tree ID:{track_id} {confidence:.2f}"
                        ...
                        if lat is not None and lon is not None:
                            ...
                            cv2.rectangle(frame, (x1, y2 + 5), (x1 + gtw + 6, y2 + gth + 10), (0, 255, 0), -1)
                        ...
                        # Generate image path
                        if lat is not None and lon is not None:
                            img_path = os.path.join(UNIQUE_DIR, f"mango_tree_{track_id}_lon_{lon}_lat_{lat}.jpg")
                        else:
                            img_path = os.path.join(UNIQUE_DIR, f"mango_tree_{track_id}.jpg")

                        # Save logic: first time OR 15% larger area
                        if track_id not in seen_ids:
                            # First time seeing this mango tree
                            seen_ids.add(track_id)
                            cv2.imwrite(img_path, crop)
                            saved_images[track_id] = {'area': current_area, 'path': img_path}
                            print(f"New mango tree saved: ID {track_id}, area: {current_area}, confidence: {confidence:.2f}")
                            ...
                            cv2.imwrite(img_path, crop)
                            saved_images[track_id] = {'area': current_area, 'path': img_path}
                            print(f"Better mango tree image saved: ID {track_id}, new area: {current_area}")
                            img_path = img_path  # Keep path for CSV
                        else:
                            img_path = ""  # No new image saved

                        # Log to CSV
                        writer.writerow([video_file, frame_num, track_id, x1, y1, x2, y2, confidence, lat, lon, img_path])
...
with open(COUNT_PATH, "w") as f:
    f.write(f"Total unique mango trees detected: {len(seen_ids)}\n")
...
print(f"Total unique mango trees found: {len(seen_ids)}")